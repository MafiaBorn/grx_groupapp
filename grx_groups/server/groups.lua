...
function groups:refreshGroupStages()
    ...
    for i=1, #self.members do
        if self.members[i] then
            ...
            TriggerClientEvent('grx_groups:client:updateGroupStage', source, self.status, self.stage)
            TriggerClientEvent('grx_groups:client:refreshGroups', source, self:getClientData(), true)
        end
    end
end
